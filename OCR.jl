...
using Images, MosaicViews, StatsBase, CairoMakie, Printf

# ╔═╡ 83330c12-6f0e-44f3-9559-4fa65b2e3941
## Input Image
img = Gray.(load("img4.png"))
...
## Square
se_square = trues(20, 20)

# ╔═╡ eaff96c7-80f3-4bbf-aad2-ce9ee79e8983
function create_diamond(size)
    se = falses(size, size)
    center = (size + 1) ÷ 2
    radius = center - 1
...
## Diamond
se_diamond = create_diamond(20)

# ╔═╡ 892ab841-3e5d-4dfc-9484-2054cb4db2df
## Dilation

# ╔═╡ 0577a9a4-7f45-413a-bd40-2a84b3b385de
function do_dilate(img::BitMatrix, se::BitMatrix = trues(3, 3))
	...
	for r in 1:img_rows, c in 1:img_rows
		## If true pixel (or 1)
		if img[r, c]
			## Union the SE image on the input image (img)
			## Like, stamping SE on img
			for sr in 1:se_rows, sc in 1:se_cols
				## If true pixel (or 1)
				if se[sr, sc]
					## Calculate where to place the mark
					out_r = r + sr - center_r
                    out_c = c + sc - center_c

					## Boundary Conditions
					if 1 <= out_r <= img_rows && 1 <= out_c <= img_cols
                		out_img[out_r, out_c] = true
					end
				end	
			end		
		end
...
# ╔═╡ 753c69e1-ac03-456c-9189-8e2d60faea96
...
dilated_img_square = Gray.(do_dilate_float(img_binary, se_square))

# ╔═╡ 045f24bb-c01d-422f-98f6-04749f8645fe
dilated_img_square_bit = Gray.(do_dilate(img_binary, se_square))

# ╔═╡ 416b2a72-05d9-40f3-be56-aaf427f0890b
dilated_img_diamond = Gray.(do_dilate_float(img_binary, se_diamond))

# ╔═╡ 4cc0cdc4-eb4d-4ed2-9854-e3797dc23a20
dilated_img_diamond_bit = Gray.(do_dilate(img_binary, se_diamond))

# ╔═╡ decda622-96da-4856-a660-cd765401709f
mosaicview(
	Gray.(img_binary),
	Gray.(dilated_img_square),
	Gray.(dilated_img_diamond);
	nrow=1, npad=10,
...
function do_erode(img::BitMatrix, se::BitMatrix = trues(3, 3))
    ## Get size of the image and the structuring element (SE)
    img_rows, img_cols = size(img)
    se_rows, se_cols = size(se)
...
# ╔═╡ fdc89ca3-5e92-4b14-9b24-e8594ce80bbd
...
eroded_img_square = Gray.(do_erode_float(img_binary, se_square))

# ╔═╡ 88af2fbf-fb91-43d4-a787-298166f1e881
eroded_img_square_bit = Gray.(do_erode(img_binary, se_square))

# ╔═╡ 4737ec1e-177a-437d-9755-0bbba2e7b338
eroded_img_diamond = Gray.(do_erode_float(img_binary, se_diamond))

# ╔═╡ 9ddc3364-3cbb-4caf-be13-219503829787
eroded_img_diamond_bit = Gray.(do_erode(img_binary, se_diamond))
...
## Opening
function do_opening(img::BitMatrix, se::BitMatrix = trues(3, 3))
...
## Closing
function do_closing(img::BitMatrix, se::BitMatrix = trues(3, 3))
...
# ╔═╡ 56d73eea-4f4f-4fbd-b272-fffc2652e6f0
begin
	template_A = load("A.png")
	binary_template_A_resized = imresize(template_A, (439, 639))
	binary_template_A_resized = Gray.(binary_template_A_resized) .> 0.5
end

# ╔═╡ 13bd1fd3-13c1-449e-acf7-ad90c28b955b
begin
    erosion_amount = 4
    probe_A = BitMatrix(copy(binary_template_A_resized))
    for _ in 1:erosion_amount
        probe_A = do_erode(probe_A)
    end
    probe_A = probe_A .> 0.5
end

# ╔═╡ a7e16fb6-1525-4d4c-b15b-3941dfd0b5c3
Gray.(probe_A)

# ╔═╡ e243be4b-bb02-438b-b3f3-9d6c63e0807c
test_image = load("Test3.png")

# ╔═╡ f65d91a2-9aeb-43d3-9820-a25dbf05e83c
binary_test_image = Gray.(test_image) .> 0.5

# ╔═╡ f3d904eb-75c0-4341-be6e-b846f57fea26
cleaned_test_image = do_opening(binary_test_image, trues(5, 5))

# ╔═╡ 3e4042b8-1650-4820-bfe2-9d82490fb73b
erosion_result = do_erode(cleaned_test_image, probe_A)

# ╔═╡ 0e3ae7dc-d120-432a-95ce-18774ef5ca06
Gray.(erosion_result)

# ╔═╡ 92ac65d8-eab2-4f7c-8d9a-620e09207b28
begin
...
# ╔═╡ 6ec58318-93c3-4b92-a38e-6533b940a74b
Gray.(cleaned_test_image)

# ╔═╡ ba9fadc4-f54d-4ad0-9fad-4a97d5a978e7
mosaicview(
	Gray.(binary_template_A_resized), Gray.(probe_A),
	Gray.(test_image), Gray.(erosion_result);
	nrow=2, npad=10,
)

# ╔═╡ a5355589-62c9-4d0b-8013-7eeeb95c64c2
@printf("Found %d instances of the character 'A'.\n", length(detected_locations))

# ╔═╡ 1eadf756-cff3-4600-b786-e7cbf44c394e
print(detected_locations)

# ╔═╡ cec025d1-e8b3-4909-b084-f404c6abef0d
function do_ocr(
...
manifest_format = "2.0"
project_hash = "9485254300a0183ec3d770804db8a98b2f8e0e72"
